...
interface Handlers {
  onEdit: (r: SalesReturn) => void;
  onRemove: (r: SalesReturn) => void;
  canManage: boolean;
  t: TranslateFn;
}

function buildColumns(h: Handlers): ColumnDef<SalesReturn, any>[] {
  return [
...
export function SalesReturnsPage() {
  const items = useSalesReturnsStore((s) => s.items);
  const add = useSalesReturnsStore((s) => s.add);
  const update = useSalesReturnsStore((s) => s.update);
  const remove = useSalesReturnsStore((s) => s.remove);
  ...
  const canManage = usePermission("sales.update");
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((r) =>
      [r.number, r.customerName ?? "", r.invoiceNumber ?? "", r.reason ?? ""].join(" ").toLowerCase().includes(q),
    );
  }, [items, search]);
  ...
  const handleSave = async () => {
    ...
    if (editing) {
      try {
        const updated = await salesReturnsApi().update(editing.id, input);
      ...
      try {
        const created = await salesReturnsApi().create(input);
        add(created);
  ...
  const columns = useMemo<ColumnDef<SalesReturn, any>[]>(
    () =>
      buildColumns({
        onEdit: openEdit,
  ...
  const dialogTitle = editing ? t("Edit return", "تعديل المرتجع") : t("New sales return", "مرتجع مبيعات جديد");

  return (
    <div className="space-y-6">
      <PageHeader
        title={t("Sales Returns", "مرتجعات المبيعات")}
        description={t("Manage sales returns from customers.", "إدارة مرتجعات المبيعات من العملاء.")}
      >
        {canCreate ? (
      ...
      <div className="overflow-hidden rounded-xl border bg-card">
        {loading ? (
          ...
          <DataTable
            ...
            emptyDescription={t("Create a sales return to record items returned by customers.", "أنشئ مرتجع مبيعات لتسجيل المنتجات المعادة من العملاء.")}
          />
        )}
      </div>
      ...
      <ConfirmDialog
        ...
        description={t("This will permanently remove the sales return.", "سيؤدي هذا إلى حذف مرتجع المبيعات نهائياً.")}
