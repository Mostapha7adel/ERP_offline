...
/// then hand over a configured port.
pub fn spawn_backend(app: AppHandle) {
    // Port the backend listens on; must match the frontend's API_BASE_URL.
    let port = 3000;
    ...
    let child = match cmd.spawn() {
        Ok(c) => c,
        Err(err) => {
            log_line(&format!("spawn failed: {err}"));
            return;
        }
    };
    log_line("spawned");

    // Give the backend a moment to bind its port before the UI first polls it.
    for _ in 0..30 {
