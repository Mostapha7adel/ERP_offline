...
export class BackupService {
  ...
  private async exportAll(): Promise<Record<string, unknown[]>> {
    ...
    const tables = [
      ...
      "report", "setting", "backup", "notification", "fiscalYear",
      "quote", "quoteLine", "recurringInvoice", "recurringInvoiceLine",
      "tradeNote", "tradeNoteLine",
      "batch", "loyaltyAccount", "loyaltyTransaction",
      "budget", "landedCost", "paymentVoucher",
      ...
      "paymentGatewayConfig", "paymentGatewayTransaction",
      "auditLog", "customReport", "scheduledReport", "periodClose",
      "rolePageAssignment", "userPageAssignment",
    ] as const;
  ...
  private async wipeAll(): Promise<void> {
    ...
    await runInTransaction(async () => {
      const tables = [
        ...
        "fiscalYear",
        "quoteLine", "quote", "recurringInvoiceLine", "recurringInvoice",
        "tradeNoteLine", "tradeNote",
        "loyaltyTransaction", "loyaltyAccount",
        "budget", "landedCost", "paymentVoucher",
        "salesReturnLine", "salesReturn", "purchaseReturnLine", "purchaseReturn",
        "priceListItem", "priceList",
        "deliveryNoteLine", "deliveryNote",
        "stockTransferLine", "stockTransfer",
        "serialNumber", "warranty",
        "paymentGatewayTransaction", "paymentGatewayConfig",
        "auditLog", "customReport", "scheduledReport", "periodClose",
        "rolePageAssignment", "userPageAssignment", "batch",
        "assetDepreciationRun", "currencyRate",
  ...
  private async importAll(data: Record<string, unknown[]>): Promise<void> {
    ...
    await runInTransaction(async () => {
      const deleteTables = [
        ...
        "quoteLine", "quote", "recurringInvoiceLine", "recurringInvoice",
        "tradeNoteLine", "tradeNote",
        "batch", "loyaltyTransaction", "loyaltyAccount",
        "budget", "landedCost", "paymentVoucher",
        "salesReturnLine", "salesReturn", "purchaseReturnLine", "purchaseReturn",
        "priceListItem", "priceList",
        "deliveryNoteLine", "deliveryNote",
