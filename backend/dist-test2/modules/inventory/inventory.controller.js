...
export function registerInventoryController(app) {
    ...
    typed.get("/inventory", {
        ...
        const rows = [];
        for (const s of all) {
            if (query.warehouseId && s.warehouseId !== String(query.warehouseId))
                continue;
    ...
    typed.get("/inventory/movements", {
        ...
        const rows = [];
        for (const m of all) {
            if (query.productId && m.productId !== String(query.productId))
                continue;
    ...
    typed.post("/inventory/adjustments", {
        ...
        const result = await withTransaction(async () => {
            await applyStockChange({ productId: input.productId, warehouseId: input.warehouseId, quantity: input.quantity, reason: input.reason, note: input.note }, principalId, { type: "adjustment" });
    ...
    typed.post("/inventory/transfers", {
        ...
        if (!(await warehouseRepository.findById(input.fromWarehouseId)))
            throw AppError.badRequest("Source warehouse not found");
        if (!(await warehouseRepository.findById(input.toWarehouseId)))
        ...
        const result = await withTransaction(async () => {
            for (const item of input.items) {
                ...
                await applyStockChange({ productId: item.productId, warehouseId: input.fromWarehouseId, quantity: -item.quantity, reason: "transfer" }, principalId, { type: "transfer-out" });
                await applyStockChange({ productId: item.productId, warehouseId: input.toWarehouseId, quantity: item.quantity, reason: "transfer" }, principalId, { type: "transfer-in" });
            }
            return { success: true };
        });
