...
export function registerReportsController(app) {
    ...
    typed.get("/reports/sales", {
        ...
        const invoices = all.filter((inv) => inv.type === "sales" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
    ...
    typed.get("/reports/purchases", {
        ...
        const { from, to } = dateRange(request.query);
        const all = await invoiceRepository.findAll();
        const invoices = all.filter((inv) => inv.type === "purchase" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
    ...
    typed.get("/reports/profit-loss", {
        ...
        const salesInvoices = allInvoices.filter((inv) => inv.type === "sales" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
        const purchaseInvoices = allInvoices.filter((inv) => inv.type === "purchase" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
    ...
    typed.get("/reports/aging", {
        ...
        const type = request.query.type ?? "receivable";
        const now = Date.now();
        const bucket = (due, total = 0) => {
            if (!due)
                return "current";
        ...
        const all = await invoiceRepository.findAll();
        const invoices = all.filter((inv) => inv.type === (type === "receivable" ? "sales" : "purchase") && inv.status !== "void" && inv.total > inv.paidAmount);
    ...
    typed.get("/reports/tax", {
        ...
        const { from, to } = dateRange(request.query);
        const all = await invoiceRepository.findAll();
        const sales = all.filter((inv) => inv.type === "sales" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
        const purchases = all.filter((inv) => inv.type === "purchase" && inv.status !== "void" && inv.invoiceDate >= from && inv.invoiceDate <= to);
