...
import { companySettingsSchema, preferencesSchema, } from "./settings.schema.js";
...
export class SettingsService {
    ...
    async updateCompany(input, audit) {
    ...
    async updatePreferences(input, audit) {
    ...
    async updateAll(input, audit) {
        let company = await this.getCompany();
        let preferences = await this.getPreferences();
        if (input.company)
            company = await this.updateCompany(input.company, audit);
        if (input.preferences)
            preferences = await this.updatePreferences(input.preferences, audit);
