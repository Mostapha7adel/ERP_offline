...
import { accountCreateSchema, journalCreateSchema, } from "./accounting.schema.js";
...
export class AccountingService {
    ...
    async updateAccount(id, input, audit) {
        ...
        if (input.code) {
            ...
            if (clash && clash.id !== id)
                throw AppError.conflict(`Account code "${input.code}" already exists`);
        }
    ...
    async createJournal(input, audit) {
    ...
    async getLedger(accountCode) {
        const account = await accountRepository.findByCode(accountCode);
        if (!account)
            throw AppError.notFound(`Account "${accountCode}" not found`);
